...
```
~/.pandi/agent/sessions/--<path>--/<timestamp>_<uuid>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.pandi/agent/sessions/`.

Pandi also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, Pandi uses the `trash` CLI to avoid permanent deletion.
...
Source on GitHub ([pandi-code](https://github.com/andrestobelem/pandi-code)):
- [`packages/coding-agent/src/core/session-manager.ts`](https://github.com/andrestobelem/pandi-code/blob/main/packages/coding-agent/src/core/session-manager.ts) - Session entry types and SessionManager
- [`packages/coding-agent/src/core/messages.ts`](https://github.com/andrestobelem/pandi-code/blob/main/packages/coding-agent/src/core/messages.ts) - Extended message types (BashExecutionMessage, CustomMessage, etc.)
- [`packages/ai/src/types.ts`](https://github.com/andrestobelem/pandi-code/blob/main/packages/ai/src/types.ts) - Base message types (UserMessage, AssistantMessage, ToolResultMessage)
- [`packages/agent/src/types.ts`](https://github.com/andrestobelem/pandi-code/blob/main/packages/agent/src/types.ts) - AgentMessage union type

For TypeScript definitions in your project, inspect `node_modules/pandi-code/dist/` and `node_modules/@earendil-works/pi-ai/dist/`.
...
### Base Message Types (from Pandi-ai)
...
### Extended Message Types (from Pandi-coding-agent)
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if Pandi-generated (legacy field name)

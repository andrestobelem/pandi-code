> Pandi can create themes. Ask it to build one for your setup.
...
Pandi loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.pandi/agent/themes/*.json`
- Project: `.pandi/themes/*.json` (only after the project is trusted)
...
On first run, Pandi detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.pandi/agent/themes
vim ~/.pandi/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/andrestobelem/pandi-code/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, Pandi reloads it automatically for immediate visual feedback.
...
Pandi uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, Pandi falls back to the nearest approximation.

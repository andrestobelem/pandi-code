> Pandi can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to Pandi's agent capabilities. Use it to embed Pandi in other applications, build custom interfaces, or integrate with automated workflows.
...
```typescript
import { createAgentSession, SessionManager } from "pandi-code";
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.pandi/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.pandi/extensions/`)
- Project skills:
  - `.pandi/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.pandi/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.pandi/agent/skills/`)
  - `~/.agents/skills/`
...
import { ModelRuntime } from "pandi-code";
...
// Default: uses ~/.pandi/agent/auth.json and ~/.pandi/agent/models.json
const modelRuntime = await ModelRuntime.create();
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "pandi-code";
...
The `edit` tool returns `details.diff` for Pandi's TUI display and `details.patch` as a standard unified patch for SDK consumers.

```typescript
import { createAgentSession } from "pandi-code";
...
import { createAgentSession, defineTool } from "pandi-code";
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.pandi/agent/extensions/`, `.pandi/extensions/`, and settings.json extension sources.
...
```typescript
import type { InlineExtension } from "pandi-code";
...
```typescript
import { createEventBus, DefaultResourceLoader } from "pandi-code";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "pandi-code";
...
1. Global: `~/.pandi/agent/settings.json`
2. Project: `<cwd>/.pandi/settings.json`
...
// Auth and Models
ModelRuntime // implements `@earendil-works/pi-ai` Models and owns credential storage

...
Pandi saves conversations as sessions so you can continue work, branch from earlier turns, and revisit previous paths.

## Session Storage

Sessions auto-save to `~/.pandi/agent/sessions/`, organized by working directory. Each session is a JSONL file with a tree structure.
...
pandi --session <path|id> # Use a specific session file or partial session ID
pandi --fork <path|id>    # Fork a session file or partial session ID into a new session
```
...
`/resume` opens an interactive session picker for the current project. `pandi -r` opens the same picker at startup.
...
When available, Pandi uses the `trash` CLI for deletion instead of permanently removing files.
...
```bash
pandi --name "Refactor auth module"
pandi --name "CI audit" -p "Review this build failure"
```

Named sessions are easier to find in `/resume` and `pandi -r`.
...
When `/tree` switches away from one branch to another, Pandi can summarize the abandoned branch and attach that summary at the new position. This preserves important context from the path you left without replaying the whole branch.

...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".pandi", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".pandi", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".pandi", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".pandi"), { recursive: true });
			writeFileSync(join(cwd, ".pandi", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".pandi"), { recursive: true, force: true });

...
export const meta = {
  name: 'map-reduce',
  description: 'Map-reduce with hierarchical (recursive) reduce: cheap per-chunk map under an evidence contract, then merge map outputs in batches round-by-round until one summary-of-summaries remains — scales past a single context window (map-reduce)',
  phases: [
    { title: 'Source' },
    { title: 'Map' },
    { title: 'Reduce' },
...
if (Array.isArray(input?.items) && input.items.length) {
  ...
  units = chunkList;
  source = 'content';
  log('source=content split into chunks ' + JSON.stringify({ chunkChars, chunkCount: chunkList.length, totalChars: text.length }));
} else {
...
if (work.length < totalUnits) {
  log('map width cap applied — COVERAGE TRIMMED ' + JSON.stringify({
    mapping: work.length, total: totalUnits, dropped: totalUnits - work.length, maxChunks,
...
const failedChunks = work.map((_, i) => i + 1).filter((_, i) => { const m = mapped[i]; return !m || typeof m.output !== 'string'; });
...
log('map complete ' + JSON.stringify({
  total: work.length,
  completed: mapCount,
...
const runRound = async (level) => {
  ...
  if (failedBatches > 0) log('reduce round partial failure — COVERAGE PRESERVED (inputs carried forward) ' + JSON.stringify({ round: reduceRounds, failedBatches }));
  log('reduce round ' + JSON.stringify({ round: reduceRounds, in: inCount, out: next.length, batches: batches.length, failedBatches }));
  return next;
};
...
while (level.length > 1 && reduceRounds < maxRounds) {
  ...
  if (level.length >= inCount) {
    stuck = true;
    log('reduce STUCK: no progress this round ' + JSON.stringify({ round: reduceRounds, in: inCount, out: level.length }));
    break;
  }
}
...
if (level.length > 1) {
  ...
  log('forced drain (bounded batched, NOT a flat merge) ' + JSON.stringify({ reason, survivors: level.length, reduceBatch, maxRounds }));
  ...
  while (level.length > 1 && drainPasses < drainCap) {
    ...
    level = await runRound(level);
    if (level.length >= inCount) {
      log('forced drain STUCK: no progress ' + JSON.stringify({ pass: drainPasses, in: inCount, out: level.length }));
      break;
    }
  }
}

const result = level.length >= 1 && level[0] != null
  ? level[0]
  : 'ERROR: reduce produced no result; ' + level.length + ' unmerged partial(s) remain (see logs).';

log('map-reduce done ' + JSON.stringify({ chunks, mapCount, withFindings: findings.length, reduceRounds, failedChunks: failedChunks.length, survivors: level.length }));

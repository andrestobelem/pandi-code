...
export const meta = {
  name: 'judge-escalate',
  description: 'Generate candidates from distinct angles, judge with a typed verdict, adaptively escalate only when confidence is low (generate-and-filter)',
  phases: [
    { title: 'Generate' },
    { title: 'Judge' },
    { title: 'Synthesize' },
...
if (maxEscalations !== rawMaxEscalations) {
  log(`maxEscalations requested=${rawMaxEscalations} normalized to ${maxEscalations} (bounds [0, ${MAX_ESCALATIONS}])`);
}

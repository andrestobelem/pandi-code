...
export const meta = {
  name: 'guardrails',
  description: 'Cheap input/output guardrails with a tripwire that HALTS; wrap any workflow via protect:{name,args} or validate a single artifact (guardrails)',
  phases: [
    { title: 'Input' },
    { title: 'Run' },
    { title: 'Output' },
...
if (content == null && !protect) throw new Error('Pass { content } to validate, or { protect: { name, args } } to wrap a workflow.');
...
const GUARD = {
  ...
  properties: {
    tripped: { type: 'boolean', description: 'true ONLY if the content CLEARLY violates the rule; default false when unsure' },
    reason: { type: 'string', description: 'one sentence: how it violates (or why it is fine)' },
    evidence: { type: 'string', description: 'the quoted span / fact that triggers the rule, or INSUFFICIENT_EVIDENCE' },
  },
};
...
async function runGuards(stage, role, text, ruleList) {
  ...
  const all = verdicts.map((v, i) => (v && typeof v.tripped === 'boolean')
    ? v
    : { rule: ruleList[i], tripped: strict, reason: strict ? 'guard failed -> fail-closed (strict)' : 'guard failed -> treated as not tripped', evidence: 'INSUFFICIENT_EVIDENCE', failed: true });
  const failed = all.filter((v) => v.failed).length;
  if (failed) log(`${stage} guards: ${failed}/${ruleList.length} failed (${strict ? 'strict -> tripped' : 'lenient -> not tripped'})`);
...
phase('Input');
const inGuards = await runGuards('input', 'input-guard', content ?? protect.args, inputRules.length ? inputRules : rules);
...
log('guardrails PASS ' + JSON.stringify({ protect: protect.name, inputChecks: inGuards.ran, outputChecks: outGuards.ran }));

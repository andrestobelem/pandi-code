...
const SAMPLE = {
  ...
  properties: {
    answer: { type: 'string', description: 'the final answer, normalized to a short canonical form so equivalent answers match exactly' },
...
for (const s of valid) {
  ...
  cur.votes++; cur.samples.push(s.i);
  tally.set(key, cur);
}
const ranked = [...tally.values()].sort((a, b) => b.votes - a.votes);
const top = ranked[0];
const tied = ranked.filter((r) => r.votes === top.votes);
log(`tally ${JSON.stringify({ counted: valid.length, distinct: ranked.length, leader: top.answer, leaderVotes: top.votes, tie: tied.length > 1 })}`);
...
if (tied.length === 1) {
  ...
  const TIEBREAK = {
    ...
    required: ['answer', 'why'],
    properties: {
      answer: { type: 'string', description: 'the chosen answer, copied EXACTLY from one of the tied answers above' },
  ...
  if (!match) log(`note: tie-break judge returned off-list answer ${JSON.stringify(picked)}; falling back to ${JSON.stringify(tied[0].answer)}`);

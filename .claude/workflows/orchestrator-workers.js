...
export const meta = {
  ...
  description: 'Orchestrator–Workers: a planner decomposes an open goal into a dependency graph of subtasks, workers execute it in robustness-bounded levels (Kahn-style, cycle/stuck detection), an integrator merges results (orchestrator-workers)',
  phases: [
    { title: 'Plan' },
    { title: 'Execute' },
    { title: 'Integrate' },
...
const maxSubtasks = Math.max(1, Math.min(30, Number.isFinite(+input?.maxSubtasks) ? Math.floor(+input.maxSubtasks) : 8));
// concurrency is optional; parallel auto-manages when undefined.
const concurrency = Number.isFinite(+input?.concurrency) && +input.concurrency > 0 ? Math.floor(+input.concurrency) : undefined;
const parallelOpts = concurrency != null ? { concurrency } : undefined;

log('orchestrator-workers starting ' + JSON.stringify({ goal: compact(goal, 200), maxSubtasks, concurrency: concurrency ?? 'auto' }));
      ...
      items: {
        ...
        properties: {
          ...
          dependsOn: {
            type: 'array',
            items: { type: 'string' },
            description: 'ids of subtasks whose outputs this one needs; [] if independent. MUST form a DAG (no cycles).',
          },
        },
      },
    },
...
if (subtasks.length > maxSubtasks) {
  log('plan cap applied ' + JSON.stringify({ planned: subtasks.length, kept: maxSubtasks, dropped: subtasks.length - maxSubtasks }));
...
while (done.size < subtasks.length) {
  ...
  if (ready.length === 0) {
    ...
    const stuck = subtasks.filter((s) => !done.has(s.id)).map((s) => ({ id: s.id, waitingOn: s.dependsOn.filter((d) => !done.has(d)) }));
  ...
  ready.forEach((s, i) => {
    ...
    if (ok) {
      ...
      records.push({ id: s.id, description: s.description, dependsOn: s.dependsOn, status: 'completed', output: r.output });
    } else {
...
log('execution finished ' + JSON.stringify({
  stopReason,
...
// ---- Phase 3: INTEGRATE -----------------------------------------------------
phase('Integrate');
const coverage = `Coverage: ${subtasks.length} subtasks planned, ${completed.length} completed, ${failed.length} failed` +
  (unreached.length ? `, ${unreached.length} never reached (${JSON.stringify(unreached)})` : '') +
  `. Stop reason: ${stopReason}.`;
const gaps = [
  failed.length ? 'Failed subtasks (ran, no usable output): ' + JSON.stringify(failed.map((r) => ({ id: r.id, description: compact(r.description, 200) }))) : '',

...
const CRITIQUE = {
  ...
  properties: {
    satisfied: { type: 'boolean', description: 'true only when there are NO actionable issues left worth another round' },
    issues: {
      type: 'array',
...
while (round < maxRounds) {
  ...
  if (useJury) {
    ...
    const juryOut = await workflow('adversarial-verify', { topic: `Claims made in this draft for the task "${task}":\n\n${compact(draft, 20000)}`, skeptics: input?.skeptics ?? 3 });
    ...
    if (!verified || checked === 0) {
      critique = { satisfied: false, issues: [{ where: 'jury', problem: 'the skeptic jury found no checkable claims (draft may be too thin/vague to ground)', fix: 'make the draft concrete and falsifiable so its claims can be verified' }] };
      ...
      critique = {
        ...
        issues: killed > 0
          ? [{ where: 'claims refuted by jury', problem: `${killed} of ${checked} claim(s) could not survive a skeptic jury`, fix: 'remove or re-ground the refuted claims with evidence' }]
          : [],
      };
  ...
  if (critique?.satisfied || !(critique?.issues?.length)) { satisfied = true; break; }

...
export const meta = {
  ...
  description: 'Apply a code migration file-by-file with a green-baseline gate, per-file build/test verify, bounded repair, and rollback on failure (no broken file left behind).',
  phases: [
    { title: 'Discover' },
    { title: 'Baseline' },
    { title: 'Migrate' },
...
if (Number.isFinite(+input?.maxRepairs) && Math.floor(+input.maxRepairs) !== maxRepairs) log('maxRepairs coerced ' + JSON.stringify({ requested: +input.maxRepairs, effective: maxRepairs }));
const maxFiles = Number.isFinite(+input?.maxFiles) ? Math.max(1, Math.min(4096, Math.floor(+input.maxFiles))) : 50;
if (Number.isFinite(+input?.maxFiles) && Math.floor(+input.maxFiles) !== maxFiles) log('maxFiles coerced ' + JSON.stringify({ requested: +input.maxFiles, effective: maxFiles }));
...
if (Array.isArray(input?.files) && input.files.length) {
  ...
  const scouted = await agent(
    'Run: git ls-files. Keep only paths matching the regex ' + pattern + '. Return up to ' + maxFiles + ' of them as JSON: ' +
...
if (verifyCmd) {
  const base = await agent(
    'BEFORE any changes, run this verification command at the repo root and report whether it passes: `' + verifyCmd + '`. ' +
...
const RESULT = {
  ...
  properties: {
    file: { type: 'string' },
    status: { type: 'string', enum: ['migrated', 'failed-rolled-back', 'verify-mismatch-not-rolled-back', 'skipped', 'applied-unverified', 'dry-run-preview'] },
...
const recheck = async (n) =>
  agent(
    'Run `' + verifyCmd + '` at the repo root and report { green, evidence } where evidence quotes the decisive pass/fail output. Do NOT edit any files.',
...
for (let i = 0; i < files.length; i++) {
  ...
  if (i > 0 && verifyCmd && !dryRun) {
    ...
    if (!between?.green) {
      aborted = { reason: 'tree not green before ' + file + ' — a prior file left the tree red; stopping to avoid compounding corruption', integrity: between, stoppedBefore: file };
      break;
    }
  }
  ...
  const prompt =
    ...
    (!dryRun && verifyCmd
      ...
      : (!dryRun ? `No verify command is available, so after applying return status "applied-unverified".\n` : '')) +
  ...
  const r = await agent(prompt, node('migrate', { model: 'sonnet', effort: 'medium', schema: RESULT, label: 'migrate:' + file, phase: 'Migrate' }));
  const rec = r ?? { file, status: 'verify-mismatch-not-rolled-back', attempts: 0, notes: 'agent returned no result; NO rollback was performed by the orchestrator — ' + file + ' may be left modified on disk' };
  ...
  if (verifyCmd && !dryRun && rec.status === 'migrated') {
    ...
    if (!gate?.green) {
      ...
      rec.notes = 'orchestrator verify FAILED after agent reported migrated (status was a self-report, not gated); the orchestrator did NOT roll back, so ' + file + ' is left modified on disk: ' + (rec.notes || '');
      log(`${file}: claimed migrated but orchestrator verify red — NOT rolled back, left modified ` + JSON.stringify(gate));
    }
  }
  results.push(rec);

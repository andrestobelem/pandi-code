...
export const meta = {
  ...
  description: 'Meta-workflow: plan then generate then review then refine then write a task-specific workflow draft (workflow-factory)',
  phases: [
    { title: 'Catalog' },
    { title: 'Plan' },
    { title: 'Generate' },
...
const workflowPath = `.claude/workflows/drafts/${workflowName}.js`;
    ...
    workflows: {
      ...
      items: {
        ...
        properties: {
          ...
          kind: { type: "string", description: "lib (reusable sub-workflow) | composed (uses other workflows) | base" },
        },
      },
    },
  },
...
const known = Array.isArray(catalog?.workflows) ? catalog.workflows.filter((w) => w && w.name && w.name !== "workflow-factory") : [];
...
const PLAN = {
  type: "object",
  additionalProperties: false,
  required: ["name", "pattern", "why", "inputs", "scout", "primitives", "compose", "adapt", "promptContracts", "verification", "risks"],
  properties: {
    name: { type: "string" },
    pattern: { type: "string" },
    why: { type: "string" },
...
const REVIEW = {
  ...
  properties: {
    verdict: { type: "string", enum: ["APPROVED", "CHANGES_REQUESTED"], description: "APPROVED only when there are no concrete issues" },
    findings: {
      type: "array",
...
log("workflow-factory review " + JSON.stringify({ verdict: review?.verdict, findings: Array.isArray(review?.findings) ? review.findings.length : 0 }));
...
for (const name of planCompose) {
  if (!hasWorkflowCall(name)) log("workflow-factory provenance WARNING: plan.compose names '" + name + "' but no workflow(\"" + name + "\") call found in emitted code — composition declared then dropped");
  if (!inBasedOn(name)) log("workflow-factory provenance WARNING: plan.compose names '" + name + "' but it is absent from meta.basedOn");
}
for (const name of planAdapt) {
  if (!known.some((w) => w.name === name)) log("workflow-factory provenance WARNING: plan.adapt names '" + name + "' which is NOT in the discovered catalog — possible hallucinated provenance");
  if (!inBasedOn(name)) log("workflow-factory provenance WARNING: plan.adapt names '" + name + "' but it is absent from meta.basedOn — specialized-from provenance dropped");
}
...
if (input?.write !== false) {
  if (!codeValid) {
    log("write skipped: generated code failed validation; returning as UNVALIDATED draft " + JSON.stringify({ workflowName }));
  } else {
    phase('Write');
    try {
      const w = await agent(
      ...
      if (w == null || w.wrote !== true) {
        ...
        log("write FAILED; generated workflow returned as result instead " + JSON.stringify({ workflowName, error: writeError }));
      } else {

...
const STEP = {
  ...
  properties: {
    ...
    action: { type: 'string', description: 'one of: grep | read | find | web_search | none — the kind of observation to run next' },
    query: { type: 'string', description: 'the concrete thing to look for (pattern, path, or search query); empty when done' },
  },
};
...
const hasCitation = (s) => typeof s === 'string' && (/[\w./-]+:\d+/.test(s) || /https?:\/\//.test(s) || /[\w./-]+\.[A-Za-z0-9]+\b/.test(s));
...
while (!done && step < maxSteps) {
  ...
  try {
    ...
    trace.push({ step, thought: '', action: 'none', query: '', observation: `(reason failed: ${String(err && err.message || err)})`, uncited: false });
  ...
  if (!decided) {
    trace.push({ step, thought: '', action: 'none', query: '', observation: '(reason failed: empty result)', uncited: false });
    log(`step ${step}: reason failed (empty result)`);
    break; // converged stays false
  }

  if (decided.done || decided.action === 'none' || !decided.query) {
    trace.push({ step, thought: decided.thought ?? '', action: 'none', query: '', observation: '(actor declared done)', uncited: false });
    done = true;
  ...
  log(`step ${step}: ${decided.action} "${compact(decided.query, 80)}" -> ${nothing ? 'nothing' : uncited ? 'evidence (UNCITED)' : 'evidence'}`);
}

...
if (!task) throw new Error('Pass { task, verifyCmd?, maxTrials?, memoryCap?, actorModel?, evaluatorModel? } as workflow input.');
...
const evaluatorModel = typeof input?.evaluatorModel === 'string' && input.evaluatorModel.trim() ? input.evaluatorModel.trim() : null;
...
const VERDICT = {
  ...
  properties: {
    pass: { type: 'boolean', description: 'true ONLY if the attempt objectively satisfies the task / the verifyCmd actually passed' },
    ...
    evidence: { type: 'string', description: 'in the grounded branch, the QUOTED actual command output (exit status + relevant stdout/stderr) proving the verdict; empty string ONLY when no command was run' },
    grounded: { type: 'boolean', description: 'true ONLY if this verdict came from actually RUNNING verifyCmd via bash and observing real output (optional; defensively re-derived from quoted evidence in code)' },
  },
};
...
const REFLECTION = {
  ...
  required: ['lesson'],
  properties: {
    lesson: { type: 'string', description: 'one or two sentences: WHY the trial failed and a concrete strategy change for the NEXT full attempt (no full rewrite, no apology)' },
  },
};
...
if (!verifyCmd) log('no verifyCmd provided — Evaluator falls back to an INDEPENDENT evaluator agent (ungrounded; intrinsic-only signal per arXiv:2310.01798 (https://arxiv.org/abs/2310.01798) is weaker)');
...
while (trial < maxTrials) {
  ...
  const attempt = await agent(
    ...
    node('actor', { model: 'sonnet', effort: 'medium', label: `actor-trial-${trial}`, phase: 'Act', ...(actorTools ? { tools: actorTools } : {}) }),
  );
  ...
  if (attempt == null) {
    ...
    history.push({ trial, attempt: null, pass: false, score: 0, feedback: 'actor returned null', evidence: '', grounded: false, lesson: 'actor produced no output' });
    continue;
  }
  ...
  if (acceptablePass) {
    history.push({ trial, attempt, pass: true, score, feedback: String(verdict.feedback ?? ''), evidence: quotedOutput, grounded, lesson: null });
    passed = true;
  ...
  history.push({ trial, attempt, pass: false, score, feedback: String(verdict.feedback ?? ''), evidence: quotedOutput, grounded, lesson });
...
if (passed) {
  ...
  log('stopped at maxTrials (no passing trial) ' + JSON.stringify({ maxTrials, bestTrial: best.trial, bestScore: best.score }));
}

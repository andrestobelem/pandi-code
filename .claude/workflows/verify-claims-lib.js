...
export const meta = {
  ...
  description: 'Reusable sub-workflow: verify {claims, skeptics?} with skeptic juries, return verified/dropped/votes/coverage (lib-verify-claims)',
  phases: [
    { title: 'Verify Claims' },
  ],
};
...
if (skepticsRequested > skeptics) log('skeptics clamped down ' + JSON.stringify({ requested: skepticsRequested, used: skeptics, max: 64 }));
...
for (let i = 0; i < claims.length; i++) {
  ...
  const cast = jury.map((r) => (r && r.data && typeof r.data.refuted === 'boolean'
    ? r.data
  ...
  const record = { claim, parsedVotes: cast, failedBranches: jury.filter((r) => !(r && r.data && typeof r.data.refuted === 'boolean')).length, refutations, survived };
  votes.push(record);
  if (survived) verified.push({ ...claim, verification: record });
  else dropped.push({ ...claim, verification: record });
  log('claim verification complete ' + JSON.stringify({ index: i + 1, total: claims.length, survived, refutations, votes: cast.length, failedBranches: record.failedBranches }));
}

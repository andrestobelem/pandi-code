...
export const meta = {
  name: 'contract-gate',
  description: 'Phase-0 contract gate: structure the raw ask into an inspectable contract, value-of-information gate on ambiguity (ask or proceed), rewrite into a clean prompt, and optionally compose workflow-factory (contract-gate)',
  phases: [
    { title: 'Analyze' },
    { title: 'Gate' },
    { title: 'Rewrite' },
...
if (requestedMaxQuestions !== maxQuestions) {
  log('maxQuestions clamped to gate band ' + JSON.stringify({ requested: requestedMaxQuestions, clampedTo: maxQuestions, band: '1..3' }));
}
...
const CONTRACT = {
  ...
  properties: {
    improvedTask: { type: 'string', description: "One-sentence restatement of the user's actual intent, normalized from the raw request." },
    successCriteria: {
      type: 'array',
    ...
    assumptions: {
      ...
      items: {
        ...
        properties: {
          ...
          confidence: { type: 'string', enum: ['high', 'medium', 'low'], description: 'How likely the user accepts it.' },
          invalidatedBy: { type: 'string', description: 'What observation/answer would overturn this assumption.' },
        },
      },
    },
    nonGoals: {
    ...
    constraints: {
      type: 'array',
      description: 'Hard limits: allowed tools/providers, read-only vs mutating, cost/time budget, repo/path scope, security/data rules.',
      items: { type: 'string' },
    },
    verificationPlan: { type: 'string', description: 'How completion will be checked (tests/commands/citations/LLM-judge) against successCriteria.' },
    routingHint: {
      type: 'object',
      additionalProperties: false,
      required: ['shape', 'pattern', 'maxAgents', 'concurrency', 'rationale'],
      properties: {
        shape: { type: 'string', enum: ['trivial', 'single-agent', 'dynamic-workflow'], description: 'Recommended downstream shape.' },
        pattern: { type: 'string', description: 'Recommended catalog pattern/primitive (e.g. fan-out-and-synthesize, judge-escalate, scout-fanout), or "n/a" for trivial/single-agent.' },
        maxAgents: { type: 'number', description: 'Rough upper bound on concurrent agents (1 for trivial/single-agent).' },
        concurrency: { type: 'string', enum: ['none', 'low', 'medium', 'high'], description: 'Rough concurrency band.' },
    ...
    ambiguities: {
      ...
      items: {
        ...
        properties: {
          question: { type: 'string', description: 'The gap phrased as a question; for blocking ones, offer concrete options/defaults.' },
          blocking: { type: 'boolean', description: 'true only when impact is HIGH and no safe default exists.' },
          rationale: { type: 'string', description: 'One line: decision impact vs inferability (value-of-information vs cost).' },
          safeAssumptionIfNonBlocking: { type: 'string', description: 'The explicit default to fold in when not blocking; "" if blocking.' },
        },
      },
    },
  },
...
const LENSES = ['scope & success criteria', 'risks, constraints, irreversibility & security', 'missing inputs, hidden assumptions & ambiguity'];
let contract;
if (reviewers <= 1) {
  contract = await agent(analyzePrompt, node('analyze-contract', { model: 'sonnet', effort: 'medium', schema: CONTRACT, phase: 'Analyze' }));
  ...
  const drafts = (await parallel(
    Array.from({ length: reviewers }, (_u, i) => () =>
      agent(
        ...
        node('analyze', { model: 'sonnet', effort: 'medium', label: `analyze-${i + 1}`, schema: CONTRACT, phase: 'Analyze', cache: false }),
      ),
    ),
  )).filter(Boolean);
...
phase('Gate');
log('gate verdict ' + JSON.stringify({
...
if (blockingAll.length > 0) {
  ...
  log('contract-gate BLOCKED — emitting questions and stopping ' + JSON.stringify({ count: questions.length, totalBlocking: blockingAll.length }));
  return {
...
nonBlocking.forEach((a) => log('safe-assumption folded ' + JSON.stringify({
  for: a.question,
  assume: a.safeAssumptionIfNonBlocking,
})));
log('contract-gate PROCEED ' + JSON.stringify({ foldedAssumptions: nonBlocking.length, criteria: (contract?.successCriteria || []).length }));
...
if (improvePrompt) {
  ...
  if (!rewrittenPrompt) throw new Error('REWRITE produced an empty prompt; verdict was PROCEED but the contract was not serializable.');
...
if (planResources && routing && routing.shape === 'dynamic-workflow' && routing.pattern) {
  ...
  const RESOURCE_PLAN = {
    ...
    properties: {
      ...
      plan: { type: 'array', items: { type: 'object', additionalProperties: false, required: ['role', 'model', 'effort'], properties: {
        role: { type: 'string' },
  ...
  if (planned && Array.isArray(planned.plan) && planned.plan.length) {
    const modelsOut = {}, effortsOut = {};
    for (const p of planned.plan) { if (p && p.role) { if (p.model) modelsOut[p.role] = p.model; if (p.effort) effortsOut[p.role] = p.effort; } }
    resourcePlan = { tier: planned.tier, rationale: planned.rationale, pattern: routing.pattern, models: modelsOut, efforts: effortsOut };
    log('resourcePlan ' + JSON.stringify({ tier: resourcePlan.tier, pattern: resourcePlan.pattern, roles: Object.keys(modelsOut) }));
  } else {
...
if (generate) {
  if (routing && routing.shape !== 'dynamic-workflow') {
    ...
    log('generate=true but routing is ' + JSON.stringify({ shape: routing.shape }) + ' — skipping workflow-factory (factory should not run for non-workflow routing)');
    generated = {
      handed_off: false,
    ...
    try {
      ...
      // of killing the run.
      factoryOut = null;
      generated = { handed_off: false, reason: 'nested workflow() unavailable one level deep; returning rewrittenPrompt for manual handoff: ' + String(e && e.message || e) };

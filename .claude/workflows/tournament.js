...
if (!entrants || entrants.length === 0) {
  ...
  let angles = (Array.isArray(input?.angles) && input.angles.length)
    ? input.angles
...
while (survivors.length > 1) {
  ...
  log(`round ${round}/${totalRounds}: ${survivors.length} in, ${pairs.length} matches${bye ? " + 1 bye" : ""} ` + JSON.stringify({
    byeId: bye?.id ?? null,
  }));
  ...
  if (defaulted > 0) log(`round ${round}: ${defaulted}/${pairs.length} matches defaulted to candidate 1 (degraded result)`);

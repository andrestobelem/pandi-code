 ...
 * workflows/*.js), excluding `router` itself and anything under drafts/. The chosen
...
export const meta = {
  name: 'router',
  description: 'Routing / dispatch: discover the workflow catalog, route the request to the single best workflow, then optionally dispatch it via workflow() and return its output, or just recommend (router)',
  phases: [
    { title: 'Discover' },
    { title: 'Route' },
    { title: 'Dispatch' },
...
const maxCandidates = Math.max(1, Math.min(200, Number.isFinite(+input?.maxCandidates) ? Math.floor(+input.maxCandidates) : 60));
const reqMax = +input?.maxCandidates;
if (Number.isFinite(reqMax) && Math.floor(reqMax) !== maxCandidates) log('maxCandidates clamped ' + JSON.stringify({ requested: Math.floor(reqMax), used: maxCandidates }));
...
if (Array.isArray(input?.candidates) && input.candidates.length) {
  ...
  try {
    ...
    log('catalog scan FAILED; proceeding with empty catalog ' + JSON.stringify({ error: String(err && err.message ? err.message : err) }));
  }
  known = Array.isArray(scouted?.workflows) ? scouted.workflows.filter((w) => w && typeof w.name === 'string' && w.name) : [];
}
...
for (const w of known) {
  ...
  if (isExcluded(name)) { droppedExcluded += 1; continue; }
...
if (candidates.length > maxCandidates) {
  log('candidate cap applied ' + JSON.stringify({ shown: maxCandidates, total: candidates.length, dropped: candidates.length - maxCandidates }));
  candidates = candidates.slice(0, maxCandidates);
}
log('discovered ' + candidates.length + ' candidate workflow(s) ' + JSON.stringify({ names: candidates.map((w) => w.name) }));
...
const ROUTE = {
  ...
  properties: {
    selected: { type: 'string', description: 'EXACTLY one workflow name from the candidate list (copied verbatim), or the literal string "none" when nothing fits / the task is trivial.' },
    why: { type: 'string', description: 'Evidence-backed justification: why this is the SINGLE best fit for the request, or why nothing fits (cite the request signals you matched). One or two sentences.' },
    suggestedArgs: { type: 'object', description: 'Best-guess args object for the chosen workflow, derived from the request (map the request into the workflow\'s primary input); {} if unknown or selected="none".' },
  },
};
...
const why = typeof decision?.why === 'string' && decision.why.trim() ? decision.why.trim() : '(no rationale provided)';
const suggestedArgs = (decision?.suggestedArgs && typeof decision.suggestedArgs === 'object') ? decision.suggestedArgs : undefined;
...
const dispatchArgs = (input?.args && typeof input.args === 'object')
  ? input.args
  : (suggestedArgs ?? { request });
